...
G_DEFINE_TYPE(GxCreamMachinePaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxCreamMachinePaintBoxClass *klass)
{
...
static void set_paint_func(GxCreamMachinePaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxCreamMachinePaintBox *paint_box = GX_CREAMMACHINE_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxCreamMachinePaintBox *paint_box)
{
...
static void gx_paint_box_destroy(GtkObject *object)
{
	GxCreamMachinePaintBox *paint_box = GX_CREAMMACHINE_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_CREAMMACHINE_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
GtkWidget *gx_paint_box_new (GtkOrientation orientation, gboolean homogeneous, gint spacing)
{
	return (GtkWidget*)g_object_new(
		GX_TYPE_CREAMMACHINE_PAINT_BOX,
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxCreamMachinePaintBox *paintbox = GX_CREAMMACHINE_PAINT_BOX(wi);
