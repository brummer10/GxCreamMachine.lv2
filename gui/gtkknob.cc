...
#define GTK_CREAMMACHINE_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_CREAMMACHINE_KNOB, GtkCreamMachineKnobPrivate))

G_DEFINE_TYPE(GtkCreamMachineKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkCreamMachineKnobPrivate GtkCreamMachineKnobPrivate;

struct _GtkCreamMachineKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkCreamMachineKnobPrivate *priv = GTK_CREAMMACHINE_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkCreamMachineKnob *knob = GTK_CREAMMACHINE_KNOB(widget);
...
{
    g_assert(GTK_IS_CREAMMACHINE_KNOB(widget));
    GtkCreamMachineKnob *knob = GTK_CREAMMACHINE_KNOB(widget);
    GtkCreamMachineKnobPrivate *priv = GTK_CREAMMACHINE_KNOB_GET_PRIVATE(knob);
...
{
    GtkCreamMachineKnob *knob = GTK_CREAMMACHINE_KNOB(object);
...
/****************************************************************
 ** init the GtkCreamMachineKnobClass
 */

static void gtk_knob_class_init (GtkCreamMachineKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkCreamMachineKnobPrivate));
}
...
static void gtk_knob_init (GtkCreamMachineKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_CREAMMACHINE_KNOB, NULL ));
